...
#define SWNODE_OV8865			0
#define SWNODE_OV8865_PORT0		1
#define SWNODE_OV8865_ENDPOINT0	2

#define SWNODE_CIO2				3
#define SWNODE_CIO2_PORT0		4
#define SWNODE_CIO2_ENDPOINT0	5

#define SURFACE_OV8865_HID	"INT347A"
...
u32 camr_data_lanes[4] = {1, 2, 3, 4};

static const struct property_entry ov8865_props[] = {
	PROPERTY_ENTRY_U32("bus-type", 5), // D-PHY
	PROPERTY_ENTRY_U32("clock-lanes", 0),
	PROPERTY_ENTRY_U32_ARRAY("data-lanes", camr_data_lanes),
	PROPERTY_ENTRY_REF("remote-endpoint", &nodes[SWNODE_CIO2_ENDPOINT0]),
	{ }
};

static const struct property_entry cio2_props[] = {
	PROPERTY_ENTRY_REF("remote-endpoint", &nodes[SWNODE_OV8865_ENDPOINT0]),
	{ }
};
...
static const struct software_node nodes[] = {
	{ SURFACE_OV8865_HID },
	{ "port0", &nodes[0] },
	{ "endpoint0", &nodes[1], ov8865_props },
	{ SURFACE_CIO2_HID },
	{ "port0", &nodes[3] },
...
{
	surface_camera_remove_node(&nodes[SWNODE_OV8865_ENDPOINT0]);
	surface_camera_remove_node(&nodes[SWNODE_OV8865_PORT0]);
	surface_camera_remove_node(&nodes[SWNODE_OV8865]);
...
static int surface_camera_check_ov8865(struct device *dev, void *data)
{
	...
	if (strcmp(hid, SURFACE_OV8865_HID))
		return 0;
...
static int __init surface_camera_init(void)
{
	...
	i2c_for_each_dev(&camera.ov8865, surface_camera_check_ov8865);
	if (!camera.ov8865) {
		ret = -EPROBE_DEFER;
		goto out;
	}

	fwnode = software_node_fwnode(&nodes[SWNODE_OV8865]);
	if (!fwnode) {
		ret = -ENODEV;
		goto out;
	}
	...
	camera.ov8865->dev.fwnode = fwnode;
	((struct v4l2_subdev *)camera.ov8865->dev.driver_data)->fwnode = fwnode;
...
static void __exit surface_camera_exit(void)
{
	...
	camera.ov8865->dev.fwnode->secondary = ERR_PTR(-ENODEV);
	surface_camera_unregister_nodes();
}

...
#define SWNODE_OV2680			0
#define SWNODE_OV2680_PORT0		1
#define SWNODE_OV2680_ENDPOINT0	2

#define SWNODE_CIO2				3
#define SWNODE_CIO2_PORT0		4
#define SWNODE_CIO2_ENDPOINT0	5

#define SURFACE_OV2680_HID	"OVTI2680"
...
static const struct property_entry cio2_props[] = {
	PROPERTY_ENTRY_REF("remote-endpoint", &nodes[SWNODE_OV2680_ENDPOINT0]),
	{ }
};
...
{
	surface_camera_remove_node(&nodes[SWNODE_OV2680_ENDPOINT0]);
	surface_camera_remove_node(&nodes[SWNODE_OV2680_PORT0]);
	surface_camera_remove_node(&nodes[SWNODE_OV2680]);
...
static int surface_camera_check_ov2680(struct device *dev, void *data)
{
	...
	if (strcmp(hid, SURFACE_OV2680_HID))
		return 0;
...
static int __init surface_camera_init(void)
{
	...
	i2c_for_each_dev(&camera.ov2680, surface_camera_check_ov2680);
	if (!camera.ov2680) {
		ret = -EPROBE_DEFER;
		goto out;
	}

	fwnode = software_node_fwnode(&nodes[SWNODE_OV2680]);
	if (!fwnode) {
		ret = -ENODEV;
		goto out;
	}
	...
	camera.ov2680->dev.fwnode = fwnode;
	((struct v4l2_subdev *)camera.ov2680->dev.driver_data)->fwnode = fwnode;
...
static void __exit surface_camera_exit(void)
{
	...
	i2c_unregister_device(camera.ov2680);
	surface_camera_unregister_nodes();
}
